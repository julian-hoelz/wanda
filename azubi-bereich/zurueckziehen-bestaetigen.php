...
// Auf dieser Seite wird der angemeldete Auszubildende gefragt, ob er einen bereits angenommenen Ausbildungsnachweis wirklich zurückziehen möchte.
// Er kann auf „Ja, zurückziehen“ oder auf „Abbrechen“ klicken
...
$an_id = $_GET["an-id"]; // die ID des Ausbildungsnachweises, bei dem gefragt werden soll, ob er zurückgezogen werden soll
...
<html lang="de">
    <head>
        ...
        <title>Zurückziehen bestätigen</title>
    </head>
    <body>
        ...
        <h1>Ausbildungsnachweis zurückziehen</h1> <!-- die Überschrift der Seite -->
        <p style="margin-bottom: 30px;">Ihr Ausbildungsnachweis vom <?= htmlspecialchars($dateRange) ?> wurde von Ihrem Ausbilder angenommen. Möchten Sie ihn trotzdem zurückziehen? Er kann dann überarbeitet werden und muss neu eingereicht werden.</p>
        <button onclick="location.href = 'status-toggeln?an-id=<?= htmlspecialchars($an_id) ?>';" class="orange-button">Ja, zurückziehen</button>
